...
// 加权采样，返回索引下标
func WeightedSampling(targetNum int, totalNum int, getWeight func(int) float64) []int {
	if totalNum < targetNum || totalNum <= 0 {
		var message = fmt.Sprintf("invalid inputs: targetNum=%d, totalNum=%d", targetNum, totalNum)
		panic(message)
	}
	...
	var results = make([]int, targetNum)
